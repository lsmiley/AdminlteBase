...
class SizerManager(models.Manager):
...
class Sizer(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('sizer-update', kwargs={'pk': self.pk})
    ...
    #     # self.specialitem5_transformationhoursitem,
    #
    #     sizer_items = self.sizer_items.all()
    #     # orders = self.order.all()
    #     self.value = sizer_items.aggregate(Sum('total_price'))[
    #         'total_price__sum'] if sizer_items.exists() else 0.00
    #     self.final_value = Decimal(self.value) - Decimal(self.discount)
    #
    #     self.sum_numserver = sizer_items.aggregate(
    #         Sum('numserver'))['numserver__sum'] if sizer_items.exists() else 0.00
    #     # self.sum_numconsole =
    #     # sizer_items.aggregate(Sum('numconsole'))['numconsole__sum'] if
    #     # sizer_items.exists() else 0.00
    #     self.sum_workstation = sizer_items.aggregate(Sum('numworkstation'))[
    #         'numworkstation__sum'] if sizer_items.exists() else 0
    #     self.sum_ipaddress = sizer_items.aggregate(Sum('numipaddress'))[
    #         'numipaddress__sum'] if sizer_items.exists() else 0
    #     self.sum_endpoint = sizer_items.aggregate(Sum('total_endpoints'))[
    #         'total_endpoints__sum'] if sizer_items.exists() else 0
    #     self.sum_numconsole = sizer_items.aggregate(
    #         Sum('qty'))['qty__sum'] if sizer_items.exists() else 0
    #
    #     self.sum_wkstnshourscalc = sizer_items.aggregate(Sum('wkstnshourscalc'))[
    #         'wkstnshourscalc__sum'] if sizer_items.exists() else 0
    #     self.sum_srvshourscalc = sizer_items.aggregate(Sum('srvshourscalc'))[
    #         'srvshourscalc__sum'] if sizer_items.exists() else 0
    #     self.sum_ipshourscalc = sizer_items.aggregate(Sum('ipshourscalc'))[
    #         'ipshourscalc__sum'] if sizer_items.exists() else 0
    #     self.sum_ipshourscalc = sizer_items.aggregate(Sum('ipshourscalc'))[
    #         'ipshourscalc__sum'] if sizer_items.exists() else 0
    #     self.sum_consolehourscalc = sizer_items.aggregate(Sum('addlconhourscalc'))[
    #         'addlconhourscalc__sum'] if sizer_items.exists() else 0
    #
    #     # self.sum_hours = sizer_items.aggregate(Sum('hours'))['hours__sum'] if sizer_items.exists() else 0.00
    #     # self.sum_fte = sizer_items.aggregate(Sum('fte'))['fte__sum'] if
    #     # sizer_items.exists() else 0.00
    #
    #     self.sum_consolehourscalc = sizer_items.aggregate(Sum('addlconhourscalc'))[
    #         'addlconhourscalc__sum'] if sizer_items.exists() else 0
    #
    #     tnt_transition_values = [self.executetransitionplan_transitionhoursitem,
    #                              self.createbestpracticecustom_transitionhoursitem,
    #                              self.testinfrastructurepoc_transitionhoursitem]
    #
    ...
    #     #     Sum('executetransitionplan_transitionhoursitem' + 'createbestpracticecustom_transitionhoursitem'))['executetransitionplan_transitionhoursitem_sum'] if sizer_items.exists() else 0
    #
...
class SizerItem(models.Model):
    ...
    sizer = models.ForeignKey(
        Sizer,
        on_delete=models.CASCADE,
        related_name='sizer_items')
...
@receiver(post_delete, sender=SizerItem)
def delete_sizer_item(sender, instance, **kwargs):
