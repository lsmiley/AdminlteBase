...
class TestquestionnaireManager(models.Manager):
...
class Testquestionnaire(models.Model):
    ...
    Revenue_CHOICES = (
        ...
        ('Direct Deal - 6950-38N ', 'Direct Deal - 6950-38N '),
        ('Revenue share - 6950 -38L ( IBM Consulting)', 'Revenue share - 6950 -38L ( IBM Consulting)'),
    )
    ...
    Customer_Status_CHOICES = (
        ...
        ('Added scope Existing ESS Customer', 'Added scope Existing ESS Customer'),
        ('Customer has other services from MSS', 'Customer has other services from MSS'),
        ('Direct Deal - 6950-38N ', 'Direct Deal - 6950-38N '),
        ('Revenue share - 6950 -38L ( IBM Consulting)', 'Revenue share - 6950 -38L ( IBM Consulting)'),
    )
    ...
    # date = models.DateTimeField(default=timezone.now)
    PricingStructureSelect = models.CharField(max_length=150, choices=PricingStructure_Choices, default='Select-One')

    # Owner/Team Info
    # prepardedby = models.CharField(blank=True, max_length=150)
    # prepardedby = models.ForeignKey(User, related_name="testquestionnaires", blank=True, null=True, on_delete=models.SET_NULL)
    ...
    # Questionnaire - RFS Section
    RFS_Note = RichTextUploadingField(blank=True, null=True, default=' Copy and Paste RFS Info and any related RFS Notes here?')
    ...
    browser = TestquestionnaireManager()
    ...
    def __str__(self):
        return self.title if self.title else 'New Questionnaire'

    def get_edit_url(self):
        return reverse('update_questionnaire', kwargs={'pk': self.id})

    def get_delete_url(self):
        return reverse('delete_questionnaire', kwargs={'pk': self.id})
...
class TestquestionnaireItem(models.Model):
    product = models.ForeignKey(Product, on_delete=models.PROTECT)
    testquestionnaire = models.ForeignKey(
        Testquestionnaire,
